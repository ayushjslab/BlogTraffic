          ...
          return (
            <Link href={item.href} key={item.label}>
              <motion.div
                whileHover={{ x: 4 }}
                whileTap={{ scale: 0.98 }}
                className={`relative flex items-center gap-2 mt-1 p-3.5 rounded-xl cursor-pointer transition-all duration-300 group/item ${
                  isActive ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-200 hover:bg-white/3'
                }`}
              >
                ...
                <item.icon 
                  size={22} 
                  className={`min-w-[22px] transition-colors duration-300 ${
      ...
      <div className="p-4 mt-auto border-t border-white/5 bg-white/1">
        <div className="space-y-2">
          ...
          <Link href="/dashboard/billing">
            <motion.div 
              className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-colors group/bill ${
                pathname === '/dashboard/billing' ? 'bg-white/5 text-white' : 'text-gray-500'
              }`}
            >
          ...
          {/* Profile Pod */}
          <div className="flex items-center gap-3 p-2 rounded-2xl bg-white/3 border border-white/5 hover:border-white/10 transition-colors">
            ...
            <AnimatePresence>
              {isHovered && (
                ...
                >
                  ...
                  <span className="text-[9px] text-gray-500 truncate uppercase tracking-widest font-black">Pro Member</span>
                </motion.div>
              )}
            </AnimatePresence>
            {isHovered && (
                <LogOut size={16} className="text-gray-600 hover:text-red-400 cursor-pointer transition-colors ml-auto mr-2" />
            )}
          </div>
        </div>
      </div>
